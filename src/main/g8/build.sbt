...
).settings(sharedSettings).enablePlugins(SbtWeb, JavaAppPackaging).
  dependsOn(shared.jvm)

lazy val client = (project in file("js")).settings(
  scalaJSUseMainModuleInitializer := true,
  libraryDependencies ++= Seq("org.scala-js" %%% "scalajs-dom" % vScalajsDom)
).settings(sharedSettings).enablePlugins(ScalaJSPlugin, ScalaJSWeb).
  dependsOn(shared.js)
...
val vScalajsScripts= "1.1.3"
val vScalatags     = "0.7.0"
val sharedSettings = Seq(scalaVersion := vScala)
